...
graph TD
    ...
    H --> I
    I --> J[Update Config Dependencies]
    J --> K[pnpm install]
    K --> L[Update Regular Dependencies]
    L --> M[Format pnpm-workspace.yaml]
    M --> N{Custom Commands?}
...
The action executes in **13 distinct steps** (implemented in `src/main.ts`):

#### Step 1: Setup

- Parse and validate inputs (including optional `run` commands)
...
- Execute `pnpm install` to update lockfile
...
- **NEW**: Execute commands from `run` input sequentially
...
#### Step 9: Capture Lockfile State (After)
...
#### Step 11: Create Changesets (conditional)
...
- Generate detailed summary with dependency tables
...
/**
 * Parsed action inputs from action.yml
 */
export interface ActionInputs {
 readonly appId: string;
 readonly appPrivateKey: string;
 readonly branch: string; // default: "pnpm/config-deps"
 readonly configDependencies: ReadonlyArray<string>;
 readonly dependencies: ReadonlyArray<string>;
 readonly run: ReadonlyArray<string>; // NEW: Custom commands to run after updates
}
...
export const parseInputs: Effect.Effect<ActionInputs, InvalidInputError> = Effect.gen(function* () {
 ...
 if (configDeps.length === 0 && deps.length === 0) {
  return yield* Effect.fail(
   new InvalidInputError({
    field: "inputs",
    value: { configDeps, deps },
    reason: "Must specify at least one of: config-dependencies or dependencies"
   })
  );
 }

 return { appId, appPrivateKey, branch, configDependencies: configDeps, dependencies: deps };
});
```
...
4. Set up test repository for integration testing
