...
| `peer-check` | `never` | reads the **"after"** lockfile — the artifact the PR will carry — and the suppression rules from `WorkspaceCatalogs`. Degrades to a warning on any failure, but the **gate** fails CLOSED: a rules lookup that errors becomes an `unverified` report, which withholds auto-merge rather than passing. Short-circuits before resolving `WorkspaceCatalogs` when disabled, so a disabled run never spawns the config-dependency hook replay in a consumer's repo |
