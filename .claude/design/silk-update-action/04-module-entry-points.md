...
Each step module owns one unit of the workflow and declares three things for itself: its **result type**, its **requirement channel**, and a **tagged error only if it can actually fail**. Exactly **five carry `never`** in the error channel — `custom-commands`, `peer-check`, `regular-dependencies`, `upgrade-package-manager` and `upgrade-runtimes` — which is a claim the compiler checks rather than a convention, and is verifiable by reading the five signatures. **Re-verified against all sixteen signatures** when the `peer-check` row below was added — and note this count has now silently stopped matching *twice*, once when `configure-status` landed (not one of the group) and once when `peer-check` landed (one of them). Re-derive it rather than trusting the number: `grep -lE 'Effect\.Effect<[^,]+,\s*never' src/steps/*.ts`.
...
| `peer-check` | `never` | reads the **"after"** lockfile — the artifact the PR will carry — and the suppression rules from `WorkspaceCatalogs`. Degrades to a warning on any failure, but the **gate** fails CLOSED: a rules lookup that errors becomes an `unverified` report, which withholds auto-merge rather than passing. Calls `WorkspaceCatalogs.refresh()` **before** the rules read: the assembly was memoized by release-age discovery before the run installed, and this step must judge the after-install lockfile under the after-install plugins' rules — see the time-skew note in @./05-module-library.md. Short-circuits before resolving `WorkspaceCatalogs` when disabled, so a disabled run never spawns the config-dependency hook replay in a consumer's repo |
