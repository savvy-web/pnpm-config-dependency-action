...
const makeWorkspacesLayer = (packages: ReadonlyArray<{ name: string; path: string }>) =>
	Layer.succeed(WorkspaceDiscovery, {
		...
		infoIn: vi.fn(() => Effect.die("infoIn not used in peer-sync tests")),
		listPackagesIn: vi.fn(() => Effect.die("listPackagesIn not used in peer-sync tests")),
		refreshIn: vi.fn(() => Effect.die("refreshIn not used in peer-sync tests")),
