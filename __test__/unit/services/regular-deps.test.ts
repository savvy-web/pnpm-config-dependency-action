...
const mockWorkspaces = (packages: ReadonlyArray<{ name: string; path: string }>): Layer.Layer<WorkspaceDiscovery> =>
	Layer.succeed(WorkspaceDiscovery, {
		...
		infoIn: () => Effect.die("infoIn not used in regular-deps tests"),
		listPackagesIn: () => Effect.die("listPackagesIn not used in regular-deps tests"),
		refreshIn: () => Effect.die("refreshIn not used in regular-deps tests"),
