 ...
 * identity `GitHubToken.provision` persisted in the `pre` phase. It replaced a
 * `signoffLine(appSlug?)` helper whose App-bot branch had no production caller
 * at all — `Report` invoked it with no slug — so every commit this action
 * created signed as `github-actions[bot]` while being authored by the
 * installation's own bot. These tests exist to pin the identity, which is why
 * the App-bot cases assert the exact trailer rather than its shape.
 *
 * There are **two** fallbacks at different depths, and both are covered because
 * they produce the same string for different reasons:
 *
 ...
 *    outright — no token persisted at all, which is also every unit test that
 *    does not stand up a `pre` phase.
 *
...
const tokenWith = (identity: Partial<{ appSlug: string; appName: string; appUserId: number }>): InstallationToken =>
...
describe("resolveSignoff", () => {
	it("builds the trailer from the persisted App bot identity", async () => {
		const signoff = await run(stateWith(tokenWith({ appSlug: "test-app", appName: "Test App", appUserId: 99 })));
	...
	it("omits the numeric prefix from the email when the app user id is unknown", async () => {
		const signoff = await run(stateWith(tokenWith({ appSlug: "test-app", appName: "Test App" })));
	...
	it("falls back to github-actions[bot] when the persisted token carries no app slug", async () => {
		...
		expect(await run(stateWith(tokenWith({})))).toBe(FALLBACK_TRAILER);
	});
